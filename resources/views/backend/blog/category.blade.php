...
@section('content')
  ...
  <div class="content-wrapper">
   ...
   <div class="row mart10 padd">
        ...
        <div class="col-md-4">
          <a class="btn btn-primary btn-sm float-right" href="{{url('admin/add-blog-category')}}">Add New Category</a>
        </div>
                  ...
                  <table class="table table-hover text-nowrap1">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{$cat->name}}</td>
                        <td>{{$cat->icon}}</td>
                        <td class="blog_img"><img src="{{url('public/uploads/'.$cat->image)}}" /></td>
                        <td>{{$cat->staus}}</td>
                        <td>{{$cat->created_at}}</td>
                        <td class="up_down">
                          <a href="{{url('admin/category_up_blog/'.($cat->orders_by+1)).'/'.$cat->id}}"><i class="fas fa-arrow-up"></i></a>
                        
                          @php if($cat->orders_by > 0){ @endphp
                          <a href="{{url('admin/category_up_blog/'.($cat->orders_by-1)).'/'.$cat->id}}"><i class="fas fa-arrow-down"></i></a>
                          @php } @endphp
                        {{$cat->orders_by}}</td>
                        <td>  
                          ...
                          <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{url('admin/edit-blog-category/')}}/{{$cat->id_hash}}">Edit</a>
                                <a onclick="return confirm('Are you sure you want to delete this item?');" class="dropdown-item" href="{{url('admin/delete-blog-category')}}/{{$cat->id}}"">Delete</a>
                            </div>
                          </div>
                        </td>
