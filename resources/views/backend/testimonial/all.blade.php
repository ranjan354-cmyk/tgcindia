...
@section('content')
 ...
 <th>Home Order No</th>
  ...
  <td><input type="number" onchange="getOrderNumber(this.value,'{{$pgs->id}}')" value="{{$pgs->order_by}}"></td>
                        <td>  
...
<script>
   function getOrderNumber(srno,id){
       /// console.log(id);
        $.ajax({
         url: '{{ url("admin/reviewsOrder") }}', // Use route name for better practice
        type: 'POST',
        data: {
