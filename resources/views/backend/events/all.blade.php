...
@section('content')
  ...
  <div class="content-wrapper">
   ...
   <div class="row mart10 padd">
        ...
        <div class="col-md-4">
          <a class="btn btn-primary btn-sm float-right" href="{{url('admin/add-events')}}">Add New Events</a>
        </div>
                  ...
                  <table class="table table-hover text-nowrap1">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>{{$i+1}}</td>
                        <td>{{$pgs->name}}</td>
                        <td>{{$pgs->event_date}}</td>
                        <td>@php echo $pgs->event_time; @endphp</td>
                        <td>{{$pgs->conduct_by}}</td>
                        <td>  

                          <div class="dropdown">
                            <i class="fa fa-ellipsis-v side-el " data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></i>
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{url('admin/edit-events/')}}/{{$pgs->id_hash}}">Edit</a>
                                <a onclick="return confirm('Are you sure you want to delete this item?');" class="dropdown-item" href="{{url('admin/delete-events')}}/{{$pgs->id}}"">Delete</a>
                            </div>
                          </div>
                        </td>
