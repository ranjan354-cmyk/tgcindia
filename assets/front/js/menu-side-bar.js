...
/**edurekanew/slick.min.js**/




/**edurekanew/performance/slide-menu.js**/
'use strict';
...
/**edurekanew/performance/header_side_menu_responsive.js**/
...
/**edurekanew/HomeOptimized/top_category.js**/
$(".dropdown_category_list").hover(function() {
    $(this).addClass("open");
}, function() {
    $(this).removeClass("open");
});
$('.dropdown_menu_multi_level').on('mouseover', '.dropdown_submenu_multi', function() {
    $('.dropdown_submenu_multi.active').removeClass('active');
    $(this).addClass('active');
});
$("#mobile_side_menu_triger").click(function(e) {
    e.preventDefault();
    $("#cattrigger").trigger("click");
});
...
$('.nav_category').css('overflow', 'visible');
$(".ga_top_categories_course").on("click", function() {
    ...
    var metadata = {
        ...
        pageURL: url,
        Category: category,
        CourseFeature: 'Browse Categories',
    };
    triggerEventV2('Course Clicked', metadata, true);
});
$(".ga_category_upfront").on("click", function() {
    var url = window.location.href;
    ...
    var metadata = {
        pageURL: url,
        Category: category,
        CourseFeature: 'SubNavigation',
        marketingType: marketingType,
    };
    triggerEventV2('Category Clicked', metadata, true);
});
/**edurekanew/HomeOptimized/trending_courses.js**/
var $navbar = $('#navbar');
var $more = $('#nav-more');
var $v_wrap = $('#navbar .nav');
var $h_wrap = $('#nav-more .dropdown-menu');
var breaks = [];
var pop_all_breakpoint = 767;
...
var active = $('#nav_tab_list_course > li.active').html()
  , last = $('#nav_tab_list_course > li:nth-child(2)').html();
$('.nav_tabs_course_drop li').on('click', function() {
    active = $(this).html();
    last = $('#nav_tab_list_course > li:nth-child(2)').html();
    $(this).removeClass('active').html(last);
    $('#nav_tab_list_course > li').removeClass('active');
    $('#nav_tab_list_course > li:nth-child(2)').addClass('active').html(active);
    last = active;
    updateNav();
});
$('.navbar_course_list').css('overflow', 'visible')
$('.top_slider_course').slick({
    setPosition: 0,
    dots: false,
    infinite: false,
    speed: 300,
...
$('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
    if ($(`#${currentTabCategoryId}-trending-slider`).children().length > 0) {
        $(`#${currentTabCategoryId}-trending-slider`).slick('setPosition');
    }
});
var categoriesShown = [];
var currentTabCategoryId;
function fetchTrendingCategoryCourses(trendingCategoryId) {
    if (typeof trendingCategoryId === "undefined") {
        $('#homeloadertrending').addClass('hide');
        return;
    }
    $.ajax({
        url: '/home-trending-courses/' + trendingCategoryId,
        async: true,
        type: 'GET',
        error: function(e) {
            console.log("ERRORRR!!");
            fetchTrendingCategoryCourses(trendingCategoryId);
        },
        success: function(resHtmlData) {
            categoriesShown[trendingCategoryId] = true;
            $(`#${trendingCategoryId}-trending-slider`).html(resHtmlData);
            initializeSliderForTrendingCourses(trendingCategoryId);
            $('#homeloadertrending').addClass('hide');
        }
    });
}
function initializeSliderForTrendingCourses(trendingCategoryId) {
    $(`#${trendingCategoryId}-trending-slider`).slick({
        setPosition: 0,
        dots: false,
        infinite: false,
        speed: 300,
...
function renderTrendingCourseDataFromStorage(categoryId) {
    $(`#${categoryId}-trending-slider`).html(localStorage.getItem(categoryId));
    initializeSlider(categoryId);
}
$(document).ready(function() {
    $('.trending_categories').click(function(e) {
        e.preventDefault();
        var categoryId = $(this).attr("data-categoryId");
        currentTabCategoryId = categoryId;
        if (categoriesShown[categoryId] != undefined) {
            $('#navbar').removeClass('in');
            return;
        } else {
            $('#homeloadertrending').removeClass('hide');
            fetchTrendingCategoryCourses(categoryId);
        }
        $('#navbar').removeClass('in');
        var text = $(this).contents()[0].nodeValue.trim();
        $(".navbar_brand_text").html('').append(text);
    });
});
$(".trending_courses").on("click", ".homecardmain", function() {
    var url = window.location.href;
    var name = $(this).find('h3.coursetitle').text();
    var category = $('#nav_tab_list_course > li.active').text();
    var metadata = {
        CourseName: name,
        pageURL: url,
        Category: category,
        CourseFeature: 'Trending',
    };
    triggerEventV2('Course Clicked', metadata, true);
});
$(".trending_courses").on("click", ".trending_categories", function() {
    var url = window.location.href;
    var category = $(this).text();
    var marketingType = getMarketType();
    var metadata = {
