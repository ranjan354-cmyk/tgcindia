...
class PartnerController extends Controller{
  ...
  public function view(Request $request){
    ...
    if(!empty($keyword)){
        $page = Partner::where('name', 'like', '%'.$keyword.'%')
        ->WHERE('is_deleted', '0')
        ->latest()
        ->paginate($r_page);
        ...
        $page = Partner::latest()->WHERE('is_deleted', '0')->paginate($r_page);
        $page->appends(['r_page' => $r_page]);
    }

    return view('backend.partner.all',compact('data', 'page'))->with('i', (request()->input('page', 1) - 1) * $r_page);
  }
  ...
  public function add(){
    ...
    return view('backend.partner.add', compact("data"));
  }
  ...
  public function save(Request $request){
    ...
    $pageUpd = Partner::find($insertedId);
    $pageUpd->id_hash = md5($insertedId);
    $pageUpd->save();

    return redirect()->back()->with('success', 'Partner has been Save successfully.'); 
  }

  public function check_unique($key, $value){
    $check = Partner::WHERE($key, $value)
            ->first();
    if( !empty($check->id) ){
        $value1 = $value . "1";
  ...
  public function edit($id_hash){
      ...
      $page = Partner::where('id_hash', $id_hash)->first();
      return view('backend.partner.edit', compact("data", "page"));
  }
  ...
  public function update(Request $request){
    ...
    return redirect()->back()->with('success', 'Partner has been Updated successfully.');
  }
